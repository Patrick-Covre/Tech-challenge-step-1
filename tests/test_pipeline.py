...
def test_pipeline_run(mock_pytorch_trainer_class, mock_sklearn_trainer_class, mock_mlflow_service_class, mock_data_loader_class):
    ...
    mock_loader.get_splits.return_value = (
        np.array([[1]]), np.array([[1]]), np.array([0]), np.array([0]), MagicMock()
    )
